            ...
            <ul role="list" class="divide-y divide-gray-100">
              <li v-for="user in users" :key="user.email" class="flex justify-between gap-x-6 py-5">
                <div class="flex min-w-0 gap-x-4">
                  <img class="h-12 w-12 flex-none rounded-full bg-gray-50" :src="user.imageUrl || defaultImage" alt="" />
                ...
                <div class="flex shrink-0 items-center gap-x-6">
                  <div class="hidden sm:flex sm:flex-col sm:items-end">
                    <p class="text-sm leading-6 text-gray-900">{{ user.roles[0]?.name }}</p>
                  </div>
                      ...
                      <MenuItems class="absolute right-0 z-10 mt-2 w-32 origin-top-right rounded-md bg-white py-2 shadow-lg ring-1 ring-gray-900/5 focus:outline-none">
                        <MenuItem v-slot="{ active }">
                          <a href="#" :class="[active ? 'bg-gray-50' : '', 'block px-3 py-1 text-sm leading-6 text-gray-900']">
                            View profile<span class="sr-only">, {{ user.name }}</span>
                          </a>
                        </MenuItem>
                        <MenuItem v-slot="{ active }">
                          <a href="#" :class="[active ? 'bg-gray-50' : '', 'block px-3 py-1 text-sm leading-6 text-gray-900']">
                            Message<span class="sr-only">, {{ user.name }}</span>
                          </a>
                        </MenuItem>
                      </MenuItems>
                    </transition>
